...
| INITQ-015 | P1 | 1 | 2026-07-28 to 2026-08-03 | `complete_on_master` | init | — | Correct `AGENTS.md` | **Completed 2026-08-13.** Live parallel guidance names the implemented process runner, deterministic parent CSV families, and the existing-but-unwired combined-workbook helper accurately. The stale LOC table and duplicated mapping/dashboard backlogs were removed; Git history retains the historical snapshot and the shortened section links only to each repository's owning queue. | 2026-08-13 |
...
| INITQ-017 | P1 | 2 | 2026-08-04 to 2026-08-10 | `complete_on_master` | init | — | Clear the two non-guard pre-existing test failures | **Completed 2026-08-13.** The projection-cache fixture already carries the current allocation-ledger argument and passes. The supply-assets test now verifies the augmented reference loader's maintained subtotal-map/filter flags instead of monkeypatching the removed helper. Both focused tests pass together. | 2026-08-13 |
| INITQ-018 | P1 | 2 | 2026-08-04 to 2026-08-10 | `complete_on_master` | init | — | Bound the test suite's runtime and memory | **Completed 2026-08-13.** `pytest.ini` now excludes the explicit `integration` marker by default. Real package staging, golden-data checks, full dashboard renders, and full-source loader checks remain available opt-in. Collection found 1,471 default tests and 35 deselected integration cases in 5.38 s. The first complete default run finished in 10m51s while a mapping conversion was active, with one pytest process at about 1.2 GB rather than the former six-process 1.8-3.2 GB fan-out. The agent guide records both commands and warns that a whole integration run previously exceeded 20 minutes and several GB per worker. Nine ordinary failures exposed by that first bounded run were repaired or refreshed before closure; the final default run is the completion gate. | 2026-08-13 |
| INITQ-019 | P1 | 2 | 2026-08-04 to 2026-08-10 | `complete_on_master` | init | — | Finish Phase 2 (`[14]`) — the tests, not the wiring | **Completed 2026-08-13.** Supply retained its existing focused coverage. `7bd733e` centralised transformation notebook defaults and added forwarding/default tests; `d2ba65a` did the same for electricity/heat interim while preserving Target + Current Accounts. Transfers already met the convention, so review correctly produced no code change. The six focused wrapper tests pass together without production data preparation or LEAP access. | 2026-08-13 |
...
| INITQ-021 | P1 | 3 | 2026-08-11 to 2026-08-17 | `human_decision` | init | user scoping | Scope the single-file combined-workbook merge for parallel runs | **Corrected evidence 2026-08-13:** `merge_parallel_results_workbooks()` already exists and preserves the raw Export preamble/header plus optional run manifests; synthetic structural tests cover deterministic order, duplicate keys, failed workers, and layout mismatch. It is not called by `parallel_runner.py`, and the promised byte-level comparison against a real sequential multi-economy run is not recorded. **Next:** decide whether to activate it, then run the real build-and-diff acceptance before wiring it into unattended production. Until then the supported path remains per-economy worker workbooks. | 2026-08-13 |
| INITQ-022 | P2 | 3 | 2026-08-11 to 2026-08-17 | `complete_on_master` | init | — | Extend `supply_reconciliation/parallel_merge.py` to the other diagnostic families | **Completed 2026-08-13.** `merge_parallel_diagnostic_families()` writes deterministic parent views for source diagnostics, template matching, and both F5 conservation summary/breakdown/lineage triplets. Missing economy values are filled from the isolated worker snapshot, failed workers are excluded, optional missing files yield stable empty parent views, and every worker CSV remains untouched. The full parallel-merge module passes (12 tests). Priorities 2–4 remain separately decision-gated. | 2026-08-13 |
