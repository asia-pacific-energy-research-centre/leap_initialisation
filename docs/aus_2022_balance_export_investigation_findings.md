...
The investigated seed was generated by transformation code that calculated:
...
The code-side fix now calculates Process Efficiency from the output and
feedstock maps stored on every shared transformation process record. The AUS
example is therefore:

```text
16.775 / 41.930999 = 40.005% (approximately 40.01%)
```

The shared boundary is intentional. Sector-specific builders still decide
which ESTO rows are feedstocks and which are auxiliaries, but a builder cannot
accidentally add exported auxiliary energy to Process Efficiency. Existing
auxiliary-fuel ratios remain separate and unchanged.
...
The checked-in AUS import template and generated seed both show a blank
`Scale` for
`Demand\Other loss and own use\Non specified own uses\Natural gas`,
`Activity Level`; the template's `Units` value is `Unspecified Unit`.
`validate_seed_files()` does not compare `Scale`, `Units`, or `Per...` metadata
with the template. It therefore did not diagnose the live LEAP branch
behaviour. Moreover, a new seed-to-template comparison would pass for this
specific file because both sides are blank; detecting live-area drift requires
an export/API check of the imported LEAP branch, or a template captured from an
area that retains the problematic setting.
...
2. Classify each transformation own-use/loss row as process auxiliary,
   ...
   variable. Feedstock-only Process Efficiency is now enforced code-side, but
   the classification still determines which map each row enters.
...
- Focused feedstock-efficiency, transformation, transfers, and registry tests:
  85 passed. A broader transformation sweep reached 108 passed with one
  unrelated pre-existing projection-cache failure because its monkeypatch does
  not accept the newer `allocation_ledger` keyword.
