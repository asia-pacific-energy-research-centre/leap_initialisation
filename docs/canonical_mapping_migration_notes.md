...
where those columns exist (matches existing `supply_reconciliation/demand_mapping.py` logic).

Legacy mapping touch-points found in active supply-reconciliation paths:

| # | Location | Legacy source | Target |
|---|----------|---------------|--------|
| 1 | `supply_reconciliation/config.py` `BALANCE_DEMAND_NINTH_TO_ESTO_MAPPING` -> `leap_results_dashboard_balance.DEFAULT_MAPPING_PAIRS_PATH` = `(config/master_config.xlsx, ninth_pairs_to_esto_pairs)` | master_config.xlsx | canonical `ninth_pairs_to_esto_pairs` |
...
Note: `supply_reconciliation/demand_mapping.py` (direct balance demand) ALREADY reads canonical
