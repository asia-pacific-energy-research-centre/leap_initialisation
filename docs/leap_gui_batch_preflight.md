...
- the latest matching `.leap` backup files in this repository's ignored
  `LEAP_backups/` folder;
- the latest final baseline seed under the reconciliation run outputs.

It does **not** scan, select, create, or copy into `C:\LEAP_Areas`, and it does
not read from OneDrive. LEAP owns its managed folder: after a repository-local
backup `.leap` is staged in a temporary folder and opened through LEAP, LEAP
itself installs/manages the resulting area there.
