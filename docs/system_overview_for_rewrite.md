...
- Loss/own-use inputs used by configured sectors include `10.01.02 Gas works plants`, `10.01.03 Liquefaction/regasification plants`, `10.01.05 Coke ovens`, `10.01.06 Coal mines`, `10.01.07 Blast furnaces`, and `10.01.11 Oil refineries`. `10.01.17 Non-specified own uses` was removed from this list: it has no causal link to `09.12 Non-specified transformation` activity (e.g. `12NZ`/`09_ROK` have real `10.01.17` own-use every year with no `09.12` output ever, which caused the own-use row to be silently dropped whenever `09.12` had no balanced input/output pair), so it is now sourced entirely by the `nonspecified_own_uses` proxy in `other_loss_own_use_proxy_workflow.py` instead.
...
- Enabled proxy outputs currently include `Coal mines`, `Electricity, CHP and heat plants`, `Liquefaction/regasification plants`, `Oil and gas extraction`, `Pump storage plants`, `Nuclear industry`, `Gasification plants for biogases`, `Non-specified own uses`, and `Transmission and distribution loss`.
- Disabled scaffold proxy configs currently include `Gas works plants`, `Gas-to-liquids plants`, `Coke ovens`, `Blast furnaces`, `Patent fuel plants`, `BKB/PB plants`, `Liquefaction plants (Coal to Oil)`, `Oil Refining`, `Charcoal production plants`, and `CCS`.
- `Non-specified own uses` is proxied off absolute total transformation-sector throughput (sum of every leaf `09.*` flow's input plus output, not just `09.12 Non-specified transformation`), since `10.01.17` own-use is not caused by non-specified transformation activity specifically — an economy can have one without the other (e.g. `12NZ`, `09_ROK`), which previously left the sector's own-use dropped whenever `09.12` had no balanced input/output pair.
