...
|---|---|---|---|---|
| `aus_2022_balance_structure_review_workbook_execution_prompt.md` | Implementation plus spreadsheet verification | Valid, active (added 2026-07-28) | Builds an AUS 2022 workbook that preserves the LEAP Energy Balance layout while showing actual LEAP values, red `LEAP - source` errors, blue correct source values, and a complete missing-combination audit. | Verify the corrected AUS workbook and diagnostic CSV paths first. Use `@oai/artifact-tool`, preserve source formatting, write the final workbook to the main-repository output tree, and archive this prompt after implementation. |
