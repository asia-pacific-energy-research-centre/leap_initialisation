...
| `auto_balance_mapping_pipeline_followthrough_20260825.md` | Cross-repo implementation and validation; active | Registers the synthetic `99 AUTO BALANCE` display-name authority, refreshes canonical mappings, verifies LEAP template/fuel readiness, and reruns baseline-seed checks. | Read the current `leap_mappings` mapping-system guidance; do not edit generated pair sheets or invent economy-specific LEAP IDs. |
...
| `full_initialisation_guide_creation.md` | Documentation consolidation; active | Builds one layered, modeller-led guide across clean-slate preparation, template export, seed generation, primary review, repeated detailed-model integration, checkpoint archiving, final readiness, and handover, with minimal code discussion. | Complete the source ledger first; treat supplied documents as evidence; keep the living rule-adjustment guide as a companion; approve Markdown content before creating the unbranded Word publication. |
