...
| `refining_workflow_scoped_review.md` | Plan-first implementation | Valid, active; retirement decision recorded 2026-07-28 | Implements the shared LEAP process-boundary normalization, verifies Oil Refining through the common transformation exporter, then retires the unused standalone prebuilt-workbook workflow. | Read `docs/refining_workflow_retirement_audit.md` first. Boundary fix and retirement must be separate commits; stop if a real current prebuilt-workbook user or unreproducible legacy behavior is found. |
