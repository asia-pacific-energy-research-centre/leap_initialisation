...
| `investigate_aus_2022_balance_export_for_seed_improvements.md` | Investigation, targeted implementation, and verification | Valid, active (added 2026-07-27) | Uses the limited-year AUS Reference 2022 LEAP balance export to build a read-only ESTO comparison, trace material discrepancies through mappings and post-boundary seed producers, fix only proven defects, and add durable early-warning checks. | Start from a branch containing the baseline-seed balance diagnostic. The representative file is Reference/base-year only: do not claim Target/future-year verification, invent aggregate allocations, edit the input workbook, or launch a full reconciliation run without user approval. |
