...
The portable manifest now stages `common_esto_dashboard_guide.py`,
`common_esto_dashboard_emissions.py`, and their guide/factor configuration
assets. This closes the deployment-only import failures found by a local Russia
REF workbook-and-dashboard run after those modules became renderer dependencies.
The dashboard and mapping repository pins, plus the three mapping tables
regenerated by the final stage-3 run, were refreshed in the same checkpoint so
the portable package describes one coherent source state.
