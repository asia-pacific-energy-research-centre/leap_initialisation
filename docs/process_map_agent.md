...
2. `codebase/supply_reconciliation/results_saver.py` (4,429 lines) — the actual
...
3. `codebase/supply_reconciliation/config.py` — every config constant and
   preset-relevant sentinel (`_ModuleCapRule`, `_resolve_module_cap_rule`,
   etc.). The workflow wrapper does `from codebase.supply_reconciliation.config
...
| `codebase/supply_reconciliation/demand_mapping.py` | — | Demand-mapping/balance-demand loading helpers: `load_balance_demand_inputs`, `load_direct_leap_demand_inputs`, mapping-status/coverage builders. |
...
| `codebase/functions/baseline_seed_artifact_validation.py` | — | The post-write BSA-001–BSA-010 run-level gate. It reopens the saved `LEAP`/`FOR_VIEWING` sheets, reuses F2 validators, compares assembly values with serialization, and writes an audit manifest. It is shadow-only as of 2026-08-03. |
...
by `supply_reconciliation/results_saver.py`'s per-economy loop, §2.3):
...
(`codebase/supply_reconciliation/results_saver.py:3106`), reconstructed from its
...
flowchart TD
    ...
    B -->|"F4 preflight<br/>supply_reconciliation/preflight.py:1278 / :1852"| C["run_with_config()<br/>-&gt; _run_with_config_locked()<br/>economy_run_locks() acquired"]
    C --> D["run_results_linked_transformation_supply_workflow()<br/>supply_reconciliation/results_saver.py:3106"]

    D --> E1["load_balance_demand_inputs()<br/>supply_reconciliation/demand_mapping.py<br/>reads outputs/leap_results/ balance exports<br/>+ TRANSFORMATION_SUPPLY_CACHE balance_demand_cache"]
    E1 --> E2["load_results_sector_demand_table /<br/>load_results_demand_table<br/>supply_reconciliation/tables.py"]
    E2 --> F["build_transformation_balance_table /<br/>build_transformation_sector_table /<br/>build_transformation_trade_target_rows<br/>supply_reconciliation/tables.py<br/>(reads ESTO CSV + 9th CSV via transformation_workflow /<br/>transformation_analysis_utils)"]
    ...
    G --> H["build_reconciliation_table()<br/>supply_reconciliation/tables.py<br/>-&gt; apply_trade_split_between_transformation_and_supply()"]
    H --> H2{"RUN_RESET_SUPPLY_AND_TRANSFORMATION_IMPORT_EXPORT<br/>+ reset_is_effective()"}
    H2 -->|effective| H3["reset_supply_and_transformation_import_export_to_zero()<br/>supply_reconciliation/tables.py:1741 (F1 reset)"]
    H2 -->|not effective, e.g. workbook mode| H4["[WARN] reset SKIPPED —<br/>see docs/work_queue.md [17]"]
    H3 --> I
    H4 --> I["save_year_balance_tables()<br/>supply_reconciliation/balance_tables.py<br/>-&gt; outputs/balance_tables/"]
    I --> J{"CAPACITY_UNMET_PASS_MODE"}
    J -->|"baseline_seed"| J1["_run_capacity_unmet_iterative_pass /<br/>_balanced_pass, baseline_seed branch:<br/>seed only, imports=0, no residual allocation<br/>supply_reconciliation/allocation.py"]
    J -->|"results_update"| J2["_run_capacity_unmet_iterative_pass /<br/>_balanced_pass: reads LEAP balance CSVs,<br/>allocates positive/negative gaps to<br/>production, transformation capacity,<br/>import fallback per config/supply_reconciliation_config.json<br/>supply_reconciliation/allocation.py"]
    J1 --> K
    ...
    L1 --> L2["save_transformation_exports_with_split_targets()<br/>supply_reconciliation/leap_io.py<br/>-&gt; transformation_leap_imports_{economy}*.xlsx<br/>(calls transformation_workflow.py + transformation_analysis_utils.py<br/>+ hydrogen_transformation_workflow.py process registry)"]
    ...
    M --> N["_build_transformation_supply_fuel_catalog_df()<br/>supply_reconciliation/results_saver.py<br/>merges LEAP probe rows + data/leap_export_templates/*<br/>+ data/full model export *.xlsx -&gt; fuel_registry.csv"]
...
|---|---|---|---|
| Preflight | `run_preflight_compressed_projection` (`supply_reconciliation/preflight.py:1278`), `run_preflight_compressed_results_update` (`:1852`) | ESTO CSV, 9th CSV, `data/leap_export_templates/*`, `config/supply_reconciliation_config.json` | `outputs/leap_exports/supply_reconciliation/preflight_compressed_projection/`, `.../preflight_compressed_results_update/` |
| Balance-demand load | `load_balance_demand_inputs` (`supply_reconciliation/demand_mapping.py`) | LEAP balance export workbooks (`LEAP_RESULTS_TABLES_DIR`, see `outputs/leap_results/`), canonical mapping via `codebase/mappings/canonical_mapping.py` (which itself reads `leap_mappings/config/outlook_mappings_master.xlsx` per `codebase/utilities/master_config.py:12`) | Versioned `runtime/balance_demand_cache/*.parquet_cache/` bundles if `TRANSFORMATION_SUPPLY_CACHE_ENABLED` |
| Transformation/supply table build | `build_transformation_balance_table`, `build_transformation_trade_target_rows`, `prepare_projected_supply_table`, `prepare_supply_primary_table` (`supply_reconciliation/tables.py`, `supply_data_pipeline.py`) | ESTO base table (`data/00APEC_2024_low_with_subtotals.csv`, `workflow_config.ENERGY_SOURCE_ESTO_BASE_TABLE_PATH`, `workflow_config.py:156`), 9th projection table (`data/merged_file_energy_ALL_20251106.csv`, `workflow_config.py:158`), canonical/rollup mapping sheets | Versioned `runtime/transform_supply_cache/*.parquet_cache/` bundles if `TRANSFORMATION_SUPPLY_CACHE_ENABLED` |
| Reconciliation table | `build_reconciliation_table`, `apply_trade_split_between_transformation_and_supply` (`supply_reconciliation/tables.py`) | in-memory tables above | in-memory `reconciliation_table` (optionally `reconciliation_table.csv` if `RESULTS_WRITE_LEGACY_SIDECAR_FILES`) |
| Reset (F1) | `reset_supply_and_transformation_import_export_to_zero` (`supply_reconciliation/tables.py:1741`) | `reconciliation_table`, `RESET_SCOPE_*` config | zeroed `reconciliation_table` in-memory, and (if not effective) a separate `supply_transformation_zeroing_{economy}.xlsx` workbook later |
| Yearly balance tables | `save_year_balance_tables` (`supply_reconciliation/balance_tables.py`) | `reconciliation_table` | `outputs/balance_tables/` (CSV per year) |
| Capacity-unmet allocation | `_run_capacity_unmet_iterative_pass` / `_run_capacity_unmet_iterative_balanced_pass` (`supply_reconciliation/allocation.py`) | balance CSVs just written, `config/supply_reconciliation_config.json` cap/priority rules, iterative-state JSON (via `supply_reconciliation/history.py`) | updated iterative-state JSON (`runtime/` under the run's output dir), allocation ledger (in-memory, `_reset_capacity_unmet_allocation_ledger`) |
| Per-economy export generation | `_run_one_economy` closure (`supply_reconciliation/results_saver.py:3728`) calling `supply_data_pipeline.generate_supply_exports`, `save_transformation_exports_with_split_targets`, `save_transfer_exports_with_supply_overrides`, `electricity_heat_interim_workflow.build_electricity_heat_interim_rows`, `save_combined_supply_transformation_export`, `build_other_loss_own_use_proxy_workbooks_for_results_supply`, `build_aggregated_demand_workbooks_for_results_supply` | `reconciliation_table`, `transformation_process_records`, `data/leap_export_templates/leap_export_template {economy}.xlsx` | `outputs/leap_exports/supply_reconciliation/exports/*.xlsx` (per sub-workflow + combined), `outputs/leap_exports/supply_reconciliation/supporting_files/other_loss_own_use_proxy/{economy}/` |
...
| Full-model export (schema authority) | `data/full model export {DDMMYYYY}.xlsx` (most recent by filename date; several dated copies present, e.g. `full model export 15072026.xlsx`) | `_build_transformation_supply_fuel_catalog_df`, reset-scope resolution (`supply_reconciliation/preflight.py:570`), demand-zeroing branch enumeration |
...
| Capacity/cap-priority config | `config/supply_reconciliation_config.json` | `supply_reconciliation/allocation.py` (cap rules, priority order) |
...
Every extracted module (`supply_reconciliation/allocation.py`,
`supply_reconciliation/tables.py`, `supply_reconciliation/preflight.py`, etc.) does
`from codebase.supply_reconciliation.config import *`, taking its own private
...
`supply_reconciliation/preflight.py`), which is why a preset change is not "just edit a
...
1. **In-process thread pool for per-economy export generation**
   (`supply_reconciliation/results_saver.py:3904-3917`, `PARALLEL_ECONOMY_WORKERS`). Guarded
   to reject values `> 1`
   (`_resolve_parallel_economy_workers`, `supply_reconciliation/results_saver.py`, pinned by
...
- **F4** (preflight) — `supply_reconciliation/preflight.py`, run before the main pipeline
...
- **The reset is gated on the (dead) LEAP-import fill.**
  ...
  (`supply_reconciliation/results_saver.py:3557-3575`, commit `c5401a5`). A log line
...
- `tests/test_parallel_economy_workers_guard.py`,
  `tests/test_supply_reconciliation/parallel_runner.py`,
  `tests/test_parallel_economy_worker_snapshot.py`,
  `tests/test_supply_reconciliation/parallel_merge.py` — the two parallelism mechanisms in
  §5.
