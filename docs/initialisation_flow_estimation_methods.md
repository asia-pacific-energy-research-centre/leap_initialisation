...
| **10.02 Transmission and distribution losses** | **Taken from the 9th, but represented using a related activity.** | Use total positive production as the normal activity series. | The source loss value remains the yearly target. If production is unavailable, try fuel-specific total demand. If a fuel has a non-zero ESTO base-year loss but every Ninth projection value is zero, carry the signed base-year loss forward; any non-zero Ninth projection remains authoritative. |
...
|---|---|---|---|
| `10.01.01` Electricity, CHP and heat plants | **Proxy enabled** | Use the electricity and heat made by power, CHP and heat plants as the activity. A nonzero Ninth own-use projection is matched directly. When a supplied Ninth fuel series is zero in every projection year, calibrate that fuel's ESTO base-year own-use intensity against base-year plant output and apply the intensity to projected plant output; do not carry the base-year energy value flat. The optional results-update can instead read the matching LEAP balance rows. | **Implemented and tested, including the PRC zero-only projection case.** |
