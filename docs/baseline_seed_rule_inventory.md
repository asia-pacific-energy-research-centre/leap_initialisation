...
| SEED-C005 | All four IDs are required on final import rows. | A `-1`, blank, or non-numeric ID is missing. Global placeholder BranchID `99` or `100` is retained and warned, never blocking. | `supply_reconciliation_workflow.py:13261-13288` blocked only nonzero `BranchID=-1`; `functions/patch_baseline_seeds.py:726-727` defaulted IDs. | Rules SEED-003 to SEED-005; all-ID test | `confirmed_rule` (CROSS-001) |
| SEED-C006 | A nonzero or unparseable expression with any missing ID blocks import. | Constants and all `Data`/`Interp` values are inspected. Rows with global placeholder BranchID `99` or `100` remain warnings until real LEAP IDs are exported. | `supply_reconciliation_workflow.py:12381-12410,13261-13288` | SEED-004 | `confirmed_rule` |
| SEED-C007 | A zero missing-ID row can be intended as a reset, but cannot be assumed harmless because it will not address the LEAP object. It blocks by default. | Applies to constant zero, empty, and all-zero series. A deliberate exception must identify `rule_id` and at least one exact measure/logical-key/provenance field. Economy-specific unused fuel leaves are the narrow exception: an all-zero unmatched row under `Resources\`, `Stock Changes\`, or `Statistical Differences\` is omitted before final validation because those branches normally exist only where that economy has use. A nonzero unmatched row under the same roots remains a normal error. | Legacy code assumed LEAP skipped `-1` rows. | `drop_zero_only_optional_unmatched_rows`; final-writer `validation_exceptions`; optional-root zero/nonzero tests | `confirmed_rule` (CROSS-001) |
...
| SEED-C015 | Auxiliary Fuel Use is derived from absolute losses/own-use relative to output, with zero-safe handling. Demand-owned `10.01.01` uses the analogous proxy rule: nonzero Ninth targets are matched directly; a fuel with an entirely zero Ninth projection keeps its ESTO base-year own-use/output intensity against projected power/heat output, rather than carrying energy flat. | Scenario profiles inherit their source data window; the calibrated `10.01.01` fallback is evaluated independently per selected Ninth scenario. | `transformation_analysis_utils.py:1157-1215,2070-2100`; `other_loss_own_use_proxy_workflow.py`; balance sign convention in global instructions. | `transformation_series_utils.py:99-143`; focused PRC-style proxy regression | `confirmed_rule` for sign/ratio and zero-only `10.01.01` projection; source-boundary choices are existing design decisions under INIT-002 |
...
| SEED-C018 | Required scenario coverage is checked per Branch Path + Variable + Region. | Required scenarios are caller configuration. Once an aggregated-demand branch is retained because any requested scenario/year is nonzero, absent requested scenario/year source rows are emitted explicitly as zero. This includes a zero Current Accounts base-year row when only 9th Reference/Target projection data exists. | Legacy global scenario lists and workflow-specific filters vary. | SEED-010; aggregated-demand scenario-completion tests | `confirmed_rule`; scenario set is configuration |
...
| SEED-C020 | Branches must exist in the selected economy's reviewed LEAP export template before final import. | Match the template by the exact logical key `(Branch Path, Variable, Scenario, Region)`; case-insensitive name matching is diagnostic only. LEAP ID columns are installation-specific metadata and are not authoritative matching keys. The human-facing issue-group CSV reports one `missing_branch` row per economy and branch; affected variables, scenarios, IDs, values, and coverage rules are aggregated as supporting evidence rather than presented as separate root causes. | Legacy `functions/patch_baseline_seeds.py:292-472`. | SEED-011 | `confirmed_rule` (CROSS-001) |

Missing-template reports are materiality-filtered: show one row per economy and branch path only when its seed expression is nonzero in a projection year or in an ESTO vintage's final historical year. Record the qualifying projection years and the explicit ESTO labels `2024 (2022)`, `2025 (2023)`, and `2026 (2024)`. Zero-only paths are not actionable missing-branch findings.
...
| SEED-C022 | A material unknown branch path is automatically enrolled in the reviewable exception workbook as a warning-only row; non-material paths are omitted. | Material means nonzero in a projection year or an ESTO vintage's final year. The row records the triggering economies and never blocks seed generation. | The final-stage material path report; `baseline_seed_validation_exception_sets.xlsx`. | `register_material_missing_branch_findings`; `report_material_missing_seed_paths`. | `confirmed_rule` |
| SEED-C022a | Enabled exception rows are relevance-audited after completed seed runs. | The audit records each vintage/economy that triggered. Only after every intended vintage is supplied may it remove stale economy memberships and disable a row that triggered nowhere; the notes retain that pruning evidence. | Warning-ledger review requirement. | `report_material_exception_path_usage`; `audit_exception_relevance`. | `confirmed_rule` |
| SEED-C022b | A zero from the simplified source-materiality lookup cannot prune a path whose completed-run audit recorded a material seed trigger. | The ledger clears those misleading values and marks `zero filter = MAPPING INCOMPLETE — seed triggered`; refreshes do not reinsert them until the canonical mapping is repaired. | Proxy, aggregation, and multi-row mappings are not represented by the one-flow/one-fuel lookup. | `apply_zero_filter`; `refresh_exception_materiality`. | `confirmed_rule` |
...
| SEED-C025 | Transformation Process Efficiency, Exogenous Capacity, Historical Production, Output Share, and Auxiliary Fuel Use use one gross-output basis. When an output fuel is also auxiliary use, LEAP retains gross output and records the auxiliary consumption separately; the deliverable-output view subtracts same-module use, while excess remains external auxiliary input. This applies to all transformation records (including Oil Refining, Coke ovens, Blast furnaces, and LNG regasification). A fully self-consuming zero-net process temporarily preserves its gross representation because LEAP has no valid auxiliary-per-zero-output denominator. | All transformation process records and scenarios with output/auxiliary overlap. | `functions/transformation_record_builder.build_process_record`; `functions/supply_leap_io.apply_transformation_target_overrides_for_scenario`. | Feedstock-only efficiency, gross-boundary overlap, and combined refinery `09.07` + `10.01.11` reconstruction tests | `confirmed_rule`; zero-net edge case remains explicit |
