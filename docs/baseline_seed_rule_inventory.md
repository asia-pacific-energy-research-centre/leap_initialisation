...
| SEED-C020 | Branches must exist in the selected economy's reviewed LEAP export template before final import. | Case-insensitive name matching is diagnostic only; template IDs remain authoritative for that economy. The human-facing issue-group CSV reports one `missing_branch` row per economy and branch; affected variables, scenarios, IDs, values, and coverage rules are aggregated as supporting evidence rather than presented as separate root causes. | Legacy `functions/patch_baseline_seeds.py:292-472`. | SEED-011 | `confirmed_rule` (CROSS-001) |
