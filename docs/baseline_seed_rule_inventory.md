...
| SEED-C007 | A zero missing-ID row can be intended as a reset, but cannot be assumed harmless because it will not address the LEAP object. It blocks by default. | Applies to constant zero, empty, and all-zero series. A deliberate exception must identify `rule_id` and at least one exact measure/logical-key/provenance field. Temporary migration exception: rows under `Stock Changes\` and `Statistical Differences\` are retained as warnings until refreshed templates supply their IDs. | Legacy code assumed LEAP skipped `-1` rows. | `validate_exception_records`; `TEMPORARY_UNRESOLVED_BRANCH_PREFIXES`; final-writer `validation_exceptions`; scoped-exception tests | `confirmed_rule`; temporary production exception recorded in CROSS-001 |
...
| SEED-C024 | Resources paths are classified against the resolved economy template; unmatched all-zero rows may be dropped, while unmatched nonzero rows are diagnostic failures. | All scenarios/years. | `supply_reconciliation_workflow.py:11400-11455,12275-12340`. | Current supply reconciliation functions. | `confirmed_rule` |
