...
| SEED-C020 | Branches must exist in the selected economy's reviewed LEAP export template before final import. | Case-insensitive name matching is diagnostic only; template IDs remain authoritative for that economy. | Legacy `functions/patch_baseline_seeds.py:292-472`. | SEED-011 | `confirmed_rule` (CROSS-001) |
