...
| BSA-005 | Every row with a nonzero or unparseable payload resolves to valid LEAP objects; a transformation process with nonzero capacity has usable process efficiency. | hard | Reopened rows, target template, expression parser. Zero tolerance `1e-12`. Existing exact SEED exceptions may be supplied and are recorded. Explicit aggregate-demand placeholder findings retain their shared `SEED` warning status, remain visible, and never set `would_block`; they represent future LEAP-template work rather than an import-readiness stop. | SEED-003/004/005/011/013; SEED-C005–C007/C030 | ID enrichment, optional-zero omission, process-efficiency producer guard | `check_shared_seed_rules` | nonzero unresolved payload; aggregate-demand warning parity; process efficiency parity | implemented; audit |
