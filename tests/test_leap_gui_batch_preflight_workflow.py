...
def test_register_only_discovers_clean_slate_backups_by_default(tmp_path: Path) -> None:
    ...
    assert aus_job["area_selection"]["normal_route"] == "STAGE_BACKUP_OUTSIDE_LEAP_AREAS_AND_OPEN_IN_LEAP"
...
def test_staging_requires_an_explicit_reviewed_clean_slate_selection(tmp_path: Path) -> None:
    ...
    register = build_batch_register(["01_AUS"], clean_slates, seed_runs)

    assert stage_reviewed_backup_files(register, tmp_path / "staged")["staged_jobs"] == []
    register["jobs"][0]["area_selection"].update(
        {"selected_source": "clean_slate_file", "selected_path": str(clean_slate)}
    )
    result = stage_reviewed_backup_files(register, tmp_path / "staged")
    staged_path = Path(result["staged_jobs"][0]["staged_path"])
    assert staged_path.read_bytes() == b"area"

    with pytest.raises(FileExistsError):
        stage_reviewed_backup_files(register, tmp_path / "staged")


def test_staging_refuses_leap_managed_area_folder(tmp_path: Path) -> None:
    register = {"jobs": []}

    with pytest.raises(ValueError, match=r"Never stage backup files"):
        stage_reviewed_backup_files(register, Path(r"C:\LEAP_Areas\agent_staging"))
