"""Regression tests for mixed placeholder and detailed demand handling."""
...
LEAP_MAPPINGS_CODEBASE = Path(__file__).resolve().parents[2] / "leap_mappings" / "codebase"
if str(LEAP_MAPPINGS_CODEBASE) not in sys.path:
    sys.path.insert(0, str(LEAP_MAPPINGS_CODEBASE))
...
class _Context:
    ...
    def config_asset(self, role: str) -> Path | None:
        return self.components_path if role == "all_demand_aggregated_components" else None
