...
def test_equal_output_passes_and_breakdown_reproduces_headline():
    totals, breakdown, _ = build_transformation_output_conservation(_reference(), [_record()], ["Reference"])
...
def test_dropped_or_duplicated_output_is_reported(records, expected_reason):
    totals, _, _ = build_transformation_output_conservation(_reference(), records, ["Reference"])
...
def test_negative_inputs_and_zero_skeletons_do_not_enter_outputs():
    ...
    totals, _, lineage = build_transformation_output_conservation(_reference(), [record], ["Reference"])
...
def test_fan_out_is_retained_once_per_contribution_and_classified_honestly():
    ...
    totals, _, lineage = build_transformation_output_conservation(_reference(), records, ["Reference"])
    assert totals.loc[0, "produced_total"] == pytest.approx(10.0)
...
def test_empty_comparison_fails():
    with pytest.raises(ValueError, match="empty"):
        build_transformation_output_conservation(pd.DataFrame(columns=_reference().columns), [], ["Reference"])
