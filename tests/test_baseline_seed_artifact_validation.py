...
def test_nonzero_unresolved_unregistered_structure_is_blocking(tmp_path: Path) -> None:
    ...
    failures = result.findings[
        ...
        & result.findings["status"].eq("FAIL")
    ]
    assert failures["evidence"].str.contains("SEED-004").any()
    assert set(failures["structure_migration_classification"]) == {
        "not_structure_migration"
    }
    assert failures["would_block"].any()
...
def test_standalone_chp_missing_petroleum_coke_is_blocking_until_registered(
    tmp_path: Path,
) -> None:
    ...
    assert set(findings["structure_migration_classification"]) == {
        "not_structure_migration"
    }
