...
# When enabled, 09.06 gas-processing children that are active in the ESTO base
# year but absent from 9th projections are held constant at their base-year
# flow/product values. Other missing-sector families still need explicit rules.
FILL_IN_MISSING_9TH_SECTORS = False
...
# Notebook-facing defaults. None preserves the wrapper's existing runtime
# fallback instead of turning environment-dependent behavior into config.
TRANSFORMATION_NOTEBOOK_ECONOMIES = None
TRANSFORMATION_NOTEBOOK_SCENARIOS = list(GLOBAL_SCENARIOS)
TRANSFORMATION_NOTEBOOK_INCLUDE_LEAP_IMPORT = None
...
###############################
# ELECTRICITY / HEAT INTERIM WORKFLOW (WRAPPER)
###############################
ELECTRICITY_HEAT_INTERIM_NOTEBOOK_ECONOMIES = list(GLOBAL_ECONOMIES)
# Deliberately excludes Reference to preserve the existing notebook output.
ELECTRICITY_HEAT_INTERIM_NOTEBOOK_SCENARIOS = ["Target", "Current Accounts"]
ELECTRICITY_HEAT_INTERIM_NOTEBOOK_INCLUDE_LEAP_IMPORT = None
