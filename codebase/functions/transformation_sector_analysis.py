...
def analyze_lng_liquefaction_regas(
    ...
    try:
        ...
        # Kept for callers that provide historical ESTO split rows.  The
        # merged ``esto_data`` already carries the base-year values and the
        # 9th projection years, so a historical-only table must not override
        # that full series.
        _ = (esto_reference_data, esto_reference_year_cols)
        fuel_codes = {
        ...
        for component_economy in component_economies:
            # The merged table supplies the ESTO base year and 9th projections.
            # Explicit ESTO split rows are historical detail only: they must not
            # replace the projected series or lock an economy to its base-year
            # liquefaction/regasification direction.
