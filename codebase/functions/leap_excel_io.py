...
def finalise_export_df(log_df, scenario, region, base_year, final_year
    ...
    if "Scenario" in export_df.columns and year_cols:
        ...
        full_year_output_share_mask = export_df["Variable"].eq("Output Share")
        base_year_int = int(base_year)
        for year in year_cols:
            year_int = int(year)
            if year_int > base_year_int:
                export_df.loc[
                    current_accounts_mask & ~full_year_output_share_mask,
                    year_int,
                ] = pd.NA
            elif year_int == base_year_int:
                export_df.loc[
                    non_current_mask & ~full_year_output_share_mask,
                    year_int,
                ] = pd.NA
