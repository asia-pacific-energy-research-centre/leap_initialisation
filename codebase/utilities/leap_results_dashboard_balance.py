...
TOTAL_BALANCE_CHECK_COLUMNS = [
    "check_name",
    "scenario",
    "year",
    "source_workbook",
    "source_sheet",
    "leap_total_flow_label",
    "leap_total_pj",
    "esto_component_total_pj",
    "esto_direct_total_pj",
    "esto_component_difference_pj",
    "esto_direct_difference_pj",
    "ninth_component_total_pj",
    "ninth_direct_total_pj",
    "ninth_component_difference_pj",
    "ninth_direct_difference_pj",
    "status",
    "severity",
    "details",
]

TOTAL_BALANCE_CHECK_DEFAULTS = {
    "enabled": True,
    "tolerance_pj": 1e-6,
    "fail_on_error": False,
}

TOTAL_BALANCE_CHECK_SPECS = [
    {
        "check_name": "total_primary_supply",
        "leap_total_flow_names": ["total primary supply", "total primary energy supply"],
        "esto_component_prefixes": ["01", "02", "03", "04", "05", "06"],
        "esto_total_prefixes": ["07"],
        "ninth_component_prefixes": ["01", "02", "03", "04", "05", "06"],
        "ninth_total_prefixes": ["07"],
    },
    {
        "check_name": "total_transformation",
        "leap_total_flow_names": ["total transformation", "total transformation sector"],
        "esto_component_prefixes": ["09."],
        "esto_total_prefixes": ["09"],
        "ninth_component_prefixes": ["09_"],
        "ninth_total_prefixes": ["09"],
    },
    {
        "check_name": "total_final_energy_demand",
        "leap_total_flow_names": [
            "total final energy demand",
            "total final energy consumption",
            "total final energy",
        ],
        "esto_component_prefixes": ["14", "15", "16"],
        "esto_total_prefixes": ["13", "12"],
        "ninth_component_prefixes": ["14", "15", "16"],
        "ninth_total_prefixes": ["13", "12"],
    },
]
...
def _code_prefix(value: object) -> str:
    """Return the leading code token from an ESTO or 9th label."""
    text = str(value or "").strip()
    if not text:
        return ""
...
def _code_matches_any_prefix(value: object, prefixes: Sequence[str]) -> bool:
    code = _code_prefix(value)
    if not code:
        return False
    for prefix in prefixes:
        clean_prefix = str(prefix or "").strip().lower().replace(".", "_")
        if not clean_prefix:
            continue
        if clean_prefix.endswith("_"):
            if code.startswith(clean_prefix):
                return True
        elif code == clean_prefix or code.startswith(f"{clean_prefix}_"):
            return True
    return False


def _leap_total_flow_key(value: object) -> str:
    aliases = {
    ...
    parts = [part for part in str(value or "").replace("\\", "/").split("/") if str(part).strip()]
    candidates = list(reversed(parts)) if parts else [value]
...
def _balance_product_is_total(value: object) -> bool:
    if _product_is_total(value):
        return True
    text = str(value or "").strip().lower().replace("_", " ")
    text = " ".join(text.split())
    return bool(re.match(r"^\d+(?:[. ]\d+)*\s+total$", text))


def _balance_check_config(known_issues: dict[str, Any] | None) -> dict[str, Any]:
    raw = (known_issues or {}).get("total_balance_checks", {})
    if raw is False:
        raw = {"enabled": False}
    elif raw is True:
        raw = {"enabled": True}
    ...
    if not isinstance(raw, dict):
        raw = {}
    out = dict(TOTAL_BALANCE_CHECK_DEFAULTS)
    out.update(raw)
    out["enabled"] = _to_bool(out.get("enabled", True), default=True)
    out["fail_on_error"] = _to_bool(out.get("fail_on_error", False), default=False)
    tolerance = pd.to_numeric(pd.Series([out.get("tolerance_pj", 1e-6)]), errors="coerce").iloc[0]
    out["tolerance_pj"] = 1e-6 if pd.isna(tolerance) else float(tolerance)
    return out


def build_total_balance_mapping_checks(
    mapped_rows: pd.DataFrame,
    *,
    tolerance_pj: float = 1e-6,
) -> pd.DataFrame:
    """
    Compare LEAP Total-fuel balance rows with mapped ESTO and 9th component sums.

    This check keeps signs exactly as extracted. Exports, bunkers, stock
    changes, and transformation inputs are expected to arrive with their
    balance-table sign already applied.
    """
    if mapped_rows.empty:
        return pd.DataFrame(columns=TOTAL_BALANCE_CHECK_COLUMNS)

    work = mapped_rows.copy()
    for col in [
        "scenario",
        "year",
        "source_workbook",
    ...
    for col in ["is_subtotal", "leap_is_subtotal", "esto_is_subtotal", "ninth_is_subtotal"]:
        if col not in work.columns:
            work[col] = False
        work[col] = work[col].fillna(False).map(_to_bool)
    work["year"] = pd.to_numeric(work["year"], errors="coerce").astype("Int64")
    work["value_petajoule"] = pd.to_numeric(
        work.get("value_petajoule", work.get("source_value_petajoule", pd.NA)),
        errors="coerce",
    ).fillna(0.0)

    work["_leap_flow_key"] = work["leap_sector_name_full_path"].map(_leap_total_flow_key)
    missing_path = work["_leap_flow_key"].eq("")
    work.loc[missing_path, "_leap_flow_key"] = work.loc[missing_path, "leap_sector_name_original"].map(_leap_total_flow_key)
    missing_path = work["_leap_flow_key"].eq("")
    work.loc[missing_path, "_leap_flow_key"] = work.loc[missing_path, "leap_sector_name"].map(_leap_total_flow_key)
    work["_leap_fuel_key"] = work["leap_fuel_name_raw"].where(work["leap_fuel_name_raw"].ne(""), work["leap_fuel_name"])
    work["_leap_fuel_key"] = work["_leap_fuel_key"].map(_normalize_text)
    work["_group_key"] = list(
        zip(
            work["scenario"].astype(str),
            work["year"].astype("Int64").astype(str),
            work["source_workbook"].astype(str),
            work["source_sheet"].astype(str),
        )
    )

    records: list[dict[str, object]] = []
    group_cols = ["scenario", "year", "source_workbook", "source_sheet"]
    for group_key, group in work.groupby(group_cols, dropna=False):
        group_values = dict(zip(group_cols, group_key))
        non_subtotal = ~group["leap_is_subtotal"].fillna(False).astype(bool)
        for spec in TOTAL_BALANCE_CHECK_SPECS:
            total_flow_keys = {_leap_total_flow_key(value) for value in spec["leap_total_flow_names"]}
            source_total_rows = group[
                group["_leap_flow_key"].isin(total_flow_keys)
                & group["_leap_fuel_key"].isin({"total", "total fuel"})
            ].copy()
            has_relevant_component_rows = (
                group["esto_flow"].map(lambda value: _code_matches_any_prefix(value, spec["esto_component_prefixes"])).any()
                or group["leap_sector"].map(lambda value: _code_matches_any_prefix(value, spec["ninth_component_prefixes"])).any()
            )
            if source_total_rows.empty:
                if not has_relevant_component_rows:
                    continue
                records.append(
                    {
                        **group_values,
                        "check_name": spec["check_name"],
                        "leap_total_flow_label": "|".join(spec["leap_total_flow_names"]),
                        "leap_total_pj": pd.NA,
                        "esto_component_total_pj": pd.NA,
                        "esto_direct_total_pj": pd.NA,
                        "esto_component_difference_pj": pd.NA,
                        "esto_direct_difference_pj": pd.NA,
                        "ninth_component_total_pj": pd.NA,
                        "ninth_direct_total_pj": pd.NA,
                        "ninth_component_difference_pj": pd.NA,
                        "ninth_direct_difference_pj": pd.NA,
                        "status": "missing_leap_total",
                        "severity": "warning",
                        "details": "No LEAP Total-fuel row was found for this balance total.",
                    }
                )
                continue

            leap_total = float(source_total_rows["value_petajoule"].sum())
            leap_total_label = _coalesce_unique(source_total_rows["leap_sector_name_full_path"])
            if not leap_total_label:
                leap_total_label = _coalesce_unique(source_total_rows["leap_sector_name_original"])

            esto_component_mask = non_subtotal & group["esto_flow"].map(
                lambda value: _code_matches_any_prefix(value, spec["esto_component_prefixes"])
            )
            esto_direct_mask = group["esto_flow"].map(
                lambda value: _code_matches_any_prefix(value, spec["esto_total_prefixes"])
            ) & group["esto_product"].map(_balance_product_is_total)
            ninth_component_mask = non_subtotal & group["leap_sector"].map(
                lambda value: _code_matches_any_prefix(value, spec["ninth_component_prefixes"])
            )
            ninth_direct_mask = group["leap_sector"].map(
                lambda value: _code_matches_any_prefix(value, spec["ninth_total_prefixes"])
            ) & group["leap_fuel"].map(_balance_product_is_total)

            esto_component_total = float(group.loc[esto_component_mask, "value_petajoule"].sum())
            esto_direct_total = float(group.loc[esto_direct_mask, "value_petajoule"].sum())
            ninth_component_total = float(group.loc[ninth_component_mask, "value_petajoule"].sum())
            ninth_direct_total = float(group.loc[ninth_direct_mask, "value_petajoule"].sum())

            esto_component_diff = esto_component_total - leap_total
            esto_direct_diff = esto_direct_total - leap_total
            ninth_component_diff = ninth_component_total - leap_total
            ninth_direct_diff = ninth_direct_total - leap_total

            failures: list[str] = []
            warnings: list[str] = []
            if abs(esto_component_diff) > tolerance_pj:
                failures.append("ESTO component sum differs from LEAP Total-fuel row")
            if abs(ninth_component_diff) > tolerance_pj:
                failures.append("9th component sum differs from LEAP Total-fuel row")
            if not group.loc[esto_direct_mask].empty and abs(esto_direct_diff) > tolerance_pj:
                failures.append("mapped ESTO direct total row differs from LEAP Total-fuel row")
            if not group.loc[ninth_direct_mask].empty and abs(ninth_direct_diff) > tolerance_pj:
                failures.append("mapped 9th direct total row differs from LEAP Total-fuel row")
            if group.loc[esto_direct_mask].empty:
                warnings.append("No mapped ESTO direct total row was available for the optional direct-total double check")
            if group.loc[ninth_direct_mask].empty:
                warnings.append("No mapped 9th direct total row was available for the optional direct-total double check")

            status = "pass"
            severity = "info"
            details = "Total-fuel row matches mapped ESTO and 9th component sums."
            if failures:
                status = "fail"
                severity = "error"
            ...
            records.append(
                {
                    **group_values,
                    "check_name": spec["check_name"],
                    "leap_total_flow_label": leap_total_label,
                    "leap_total_pj": leap_total,
                    "esto_component_total_pj": esto_component_total,
                    "esto_direct_total_pj": esto_direct_total if not group.loc[esto_direct_mask].empty else pd.NA,
                    "esto_component_difference_pj": esto_component_diff,
                    "esto_direct_difference_pj": esto_direct_diff if not group.loc[esto_direct_mask].empty else pd.NA,
                    "ninth_component_total_pj": ninth_component_total,
                    "ninth_direct_total_pj": ninth_direct_total if not group.loc[ninth_direct_mask].empty else pd.NA,
                    "ninth_component_difference_pj": ninth_component_diff,
                    "ninth_direct_difference_pj": ninth_direct_diff if not group.loc[ninth_direct_mask].empty else pd.NA,
                    "status": status,
                    "severity": severity,
                    "details": details,
                }
    ...
    out = pd.DataFrame(records, columns=TOTAL_BALANCE_CHECK_COLUMNS)
    if out.empty:
        return pd.DataFrame(columns=TOTAL_BALANCE_CHECK_COLUMNS)
    return out.sort_values(["source_workbook", "source_sheet", "scenario", "year", "check_name"], kind="mergesort").reset_index(drop=True)


def _total_balance_checks_to_issues(total_checks: pd.DataFrame) -> pd.DataFrame:
    if total_checks.empty:
        return pd.DataFrame()
    problem = total_checks[total_checks["severity"].isin(["warning", "error"])].copy()
    if problem.empty:
        return pd.DataFrame()
    return pd.DataFrame(
        {
            "reason": "total_balance_mapping_check",
            "details": problem["details"].astype(str),
            "severity": problem["severity"].astype(str),
            "scenario": problem["scenario"].astype(str),
            "year": pd.to_numeric(problem["year"], errors="coerce").astype("Int64"),
            "source_sheet": problem["source_sheet"].astype(str),
            "source_workbook": problem["source_workbook"].astype(str),
            "leap_sector_name_full_path": problem["leap_total_flow_label"].astype(str),
            "leap_flow": problem["check_name"].astype(str),
            "leap_flow_name": problem["check_name"].astype(str),
            "leap_product": "Total",
            "leap_product_name": "Total",
            "mapping_failed": "",
            "mapping_key_sector": problem["check_name"].astype(str),
            "mapping_key_fuel": "Total",
            "mapping_candidate_rule": "total_balance_mapping_check",
            "esto_flow": "",
            "esto_product": "19 Total",
            "value_petajoule": pd.to_numeric(problem["leap_total_pj"], errors="coerce"),
        }
    )
...
def convert_leap_balances_to_esto_long_table(
    *,
    ...
    total_balance_check_tolerance_pj: float | None = None,
) -> dict[str, Any]:
    """
    ...
    ingestion = load_balance_leap_long_esto_axis(
        ...
        total_balance_check_tolerance_pj=total_balance_check_tolerance_pj,
    )
    ...
    return {
        ...
        "total_balance_checks": ingestion.get("total_balance_checks", pd.DataFrame(columns=TOTAL_BALANCE_CHECK_COLUMNS)),
...
def load_balance_leap_long(
    *,
    ...
    total_check_cfg = _balance_check_config(issues_cfg)
    if run_total_balance_checks is not None:
        total_check_cfg["enabled"] = bool(run_total_balance_checks)
    if total_balance_check_tolerance_pj is not None:
        total_check_cfg["tolerance_pj"] = float(total_balance_check_tolerance_pj)
    total_balance_checks = (
        build_total_balance_mapping_checks(
            combined,
            tolerance_pj=float(total_check_cfg["tolerance_pj"]),
        )
        if total_check_cfg["enabled"]
        else pd.DataFrame(columns=TOTAL_BALANCE_CHECK_COLUMNS)
    )
    ...
    issue_frames = [
        ...
        _total_balance_checks_to_issues(total_balance_checks),
    ]
    ...
    if total_check_cfg["fail_on_error"] and not total_balance_checks.empty:
        errors = total_balance_checks[total_balance_checks["severity"].eq("error")]
        if not errors.empty:
            preview = errors[
                [
                    "check_name",
            ...
            raise ValueError(
                "LEAP balance total checks failed. "
                f"Error rows: {len(errors):,}. Preview: {preview}"
            )
    ...
    return {
        ...
        "extraction_summary": {
            ...
            "total_balance_check_rows": int(len(total_balance_checks)),
            "total_balance_check_error_rows": int(
                len(total_balance_checks[total_balance_checks["severity"].eq("error")])
                if not total_balance_checks.empty
                else 0
            ),
            "total_balance_checks_enabled": bool(total_check_cfg["enabled"]),
        },
    }
...
def load_balance_leap_long_esto_axis(
    *,
    ...
    total_check_issues = _total_balance_checks_to_issues(total_balance_checks)
    if not total_check_issues.empty:
        issues_df = pd.concat([issues_df, total_check_issues], ignore_index=True, sort=False)
    if total_check_cfg["fail_on_error"] and not total_balance_checks.empty:
        errors = total_balance_checks[total_balance_checks["severity"].eq("error")]
        if not errors.empty:
            preview = errors[
